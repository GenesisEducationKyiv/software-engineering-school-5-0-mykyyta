...
import (
	...
	cache2 "weatherApi/monolith/internal/weather/cache"
	"weatherApi/monolith/internal/weather/chain"
	"weatherApi/monolith/internal/weather/logger"
...
func TestIntegration_CacheReader_Hit(t *testing.T) {
	...
	redisCache := cache2.NewRedisCache(db)
	reader := cache2.NewReader(&failProvider{}, redisCache, &nopMetrics{}, []string{provider})
...
func TestIntegration_CacheReader_Miss(t *testing.T) {
	...
	redisCache := cache2.NewRedisCache(db)
	reader := cache2.NewReader(&successProvider{}, redisCache, &nopMetrics{}, []string{provider})
...
func TestIntegration_CacheWriter_WritesToRedis_And_ReaderReadsIt(t *testing.T) {
	...
	redisCache := cache2.NewRedisCache(db)
	writer := cache2.NewWriter(&successProvider{}, redisCache, provider, ttl, notFoundTTL)
...
func (r *readerStub) Get(ctx context.Context, city, provider string) (domain.Report, error) {
	...
	if !ok {
		return domain.Report{}, cache2.ErrCacheMiss
	}
...
func TestIntegration_Reader_TriesProvidersInOrder(t *testing.T) {
	...
	stub := &readerStub{
		Responses: map[string]struct {
			...
			"CacheA": {Err: cache2.ErrCacheMiss},
			"CacheB": {Report: expected, Err: nil},
		},
	}

	reader := cache2.NewReader(&failProvider{}, stub, &nopMetrics{}, providerNames)
...
func TestIntegration_Reader_StopsOnRedisError(t *testing.T) {
	...
	reader := cache2.NewReader(&successProvider{}, stub, &nopMetrics{}, providerNames)
