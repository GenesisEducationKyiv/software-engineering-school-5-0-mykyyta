...
func (m *mockRepo) GetByEmail(ctx context.Context, email string) (*domain2.Subscription, error) {
	args := m.Called(ctx, email)
	if s := args.Get(0); s != nil {
		return s.(*domain2.Subscription), args.Error(1)
	}
	return nil, args.Error(1)
}

func (m *mockRepo) Create(ctx context.Context, sub *domain2.Subscription) error {
	return m.Called(ctx, sub).Error(0)
}

func (m *mockRepo) Update(ctx context.Context, sub *domain2.Subscription) error {
	return m.Called(ctx, sub).Error(0)
}

func (m *mockRepo) GetConfirmedByFrequency(ctx context.Context, frequency string) ([]domain2.Subscription, error) {
	args := m.Called(ctx, frequency)
	return args.Get(0).([]domain2.Subscription), args.Error(1)
}
...
func (m *mockEmailService) SendWeatherReport(email string, weatherReport domain2.Report, city, token string) error {
	return m.Called(email, weatherReport, city, token).Error(0)
}

type mockCityValidator struct{ mock.Mock }

func (m *mockCityValidator) GetWeather(ctx context.Context, city string) (domain2.Report, error) {
	args := m.Called(ctx, city)
	return args.Get(0).(domain2.Report), args.Error(1)
}
...
func TestSubscribe_RenewsUnsubscribedUser(t *testing.T) {
	...
	existing := &domain2.Subscription{Email: email, IsConfirmed: true, IsUnsubscribed: true}
...
func TestConfirm_ValidToken_Success(t *testing.T) {
	...
	email := "user@example.com"
	token := "valid-token"
	sub := &domain2.Subscription{Email: email, IsConfirmed: false}
...
func TestConfirm_AlreadyConfirmed(t *testing.T) {
	...
	email := "user@example.com"
	token := "valid-token"
	sub := &domain2.Subscription{Email: email, IsConfirmed: true}
...
func TestUnsubscribe_ValidToken_Success(t *testing.T) {
	...
	email := "user@example.com"
	token := "valid-token"
	sub := &domain2.Subscription{Email: email, IsUnsubscribed: false}
...
func TestUnsubscribe_AlreadyUnsubscribed(t *testing.T) {
	...
	email := "user@example.com"
	token := "token"
	sub := &domain2.Subscription{Email: email, IsUnsubscribed: true}
...
func TestGenerateWeatherReportTasks_ListFails_ReturnsError(t *testing.T) {
	...
	d.repo.On("GetConfirmedByFrequency", ctx, frequency).
		Return([]domain2.Subscription(nil), assert.AnError)
